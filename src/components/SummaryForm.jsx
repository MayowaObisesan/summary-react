...
export const DesktopSummaryFormComponent = () => {
    ...
    return (
        <Form method="GET" className="bg-white-transparent pad-x-4 pad-y-5 bg-mica z-1000 radius focus-withi:bg-27CE8E transition:background_200ms_ease_200ms dark:bg-transparent">
            <div className={"relative flex flex-row justify-center align-items-center pct:w-100 outline:0px_solid_lightgray pad-x2 border-0 border:2px_solid_D8D8D8 bg-white radius2-smt focus-within:outline:2px_solid_666666|outline-offset-2 dark:bg-transparent dark:border:1px_solid_444444"}>
                <input
                    type="text"
                    name="search_query"
                    placeholder="What should I search and summarize?"
            ...
            <div className={"flex flex-row flex-nowrap justify-start align-items-center mg-t2 mg-b1"}>
                {
                    summaryLanguages.map((eachLanguage, index) => {
                        return (
                            <div key={index} className={"lh-4 mg-r2 pad-x2 radius-sm shadow:0px-0px-2px-0px-gray cursor-pointer hover:bg-light|shadow:0px-0px-2px-0px-gray transition:background_200ms_ease dark:color-white dark:hover:bg-darkgray"}>{eachLanguage}</div>
                        )
                    })
                }
            </div>
