...
const SummaryItem = ({ itemType, groupedResult, hostName }) => {
    ...
    const SummaryItemCommon = () => {
        return (
            <>
                <div className={"relative font-12 font-medium h-3 lh-3 pad-x2 color-rebeccapurple color-191E30 text-ellipsis hover:underline-offset-3 hover:underline hover:decoration-3 dark:color-gray dark:h-5|lh-5|color-lightgray"}>{groupedResult?.title}</div>
    ...
    return (
        itemType === "group"
            ? <section class="relative flex-grow flex-noshrink flex flex-column justify-between align-items-start pct:w-100 radius-sm shadow:0px-0px-8px-1px-E4E4E4 mg-y2 bg-inherit lg:shadow:0px-0px-8px-1px-D8D8D8 dark:bg-121714 dark:shadow-unset dark:border:1px_solid_444444">
                {/* For grouped items */}
                <SummaryItemCommon />
            </section>
